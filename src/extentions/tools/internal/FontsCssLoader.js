	...
	constructor() {
		super((resolve) => {
			...
			new AllFontsLoader().then((fontFaces) => {
				let fontsCss = fontFaces.map((fontFace) => fontFace.css).join();
				const urls = Array.prototype.concat.apply(
						[],
						fontFaces.map((fontFace) => fontFace.urls)
				);
				return Thenable.all(
						urls.map((url) => xhr.getOnCacheOrHttp(url, xhrCache, 'text/plain; charset=x-user-defined' /*get binaly*/).then((responceText) => {
