...
export abstract class BaseColumn<T, V> implements ColumnTypeAPI {
  ...
  get transformCopy(): TransformRecord | unknown {
  ...
  protected transformRecordBefore(value: unknown, cell: CellAddress, grid: ListGridAPI<T>): V {
    ...
    if (this.transformCopy) {
      ...
      displayValue = transformCopy({ value, displayValue, cell, grid })
    }
    return displayValue
  }
  ...
  getTransCopyValue(value:unknown,cell: CellAddress,grid: ListGridAPI<T>):unknown{
    ...
    if (this.transformRecord) {
      ...
      displayValue = transformRecord({ value, displayValue, cell, grid })
    }
    return displayValue
  }
