    ...
    _bindMenuEvents(): void {
        ...
        handler.on(menu, 'keydown', (e) => {
            ...
            if (keyCode === KEY_ENTER) {
                ...
                const n = findPrevSiblingFocusable(item) as HTMLElement
                if (n) {
                    n.focus()
                    event.cancel(e)
                }
            } else if (keyCode === KEY_DOWN) {
                const n = findNextSiblingFocusable(item) as HTMLElement
                if (n) {
                    n.focus()
                    event.cancel(e)
                }
    ...
    _onKeydownTab(menu: HTMLUListElement, item: HTMLElement, e: KeyboardEvent): void {
        ...
        if (!e.shiftKey) {
            if (!findNextSiblingFocusable(item)) {
                let n: HTMLInputElement = menu.querySelector(`.${ ITEM_CLASSNAME }`)
                if (!isFocusable(n)) {
                    n = findNextSiblingFocusable(n) as HTMLInputElement
                }
                if (n) {
                    n.focus()
                    event.cancel(e)
            ...
            if (!findPrevSiblingFocusable(item)) {
                ...
                let n: HTMLInputElement = items[items.length - 1] as HTMLInputElement
                if (!isFocusable(n)) {
                    n = findPrevSiblingFocusable(n) as HTMLInputElement
                }
                if (n) {
                    n.focus()
                    event.cancel(e)
