...
import type {
    CellDefine, ColumnData, HeaderBodyLayoutDefine,
    HeaderCellDefine, HeaderData, LayoutDefine, LayoutMapAPI, WidthData
} from '../api'
...
export class MultiLayoutMap<T> implements LayoutMapAPI<T> {
    ...
    constructor(layout: LayoutDefine<T>) {
        ...
        const header = (this._header =
                new LayoutObjectGrid(hbLayout.header, (hd: HeaderCellDefine<T>, id: LayoutObjectId): HeaderData<T> => ({
                    id,
                    caption: hd.caption,
    ...
    getBody(col: number, row: number): ColumnData<T> | any {
        const id = this.getCellId(col, row)
