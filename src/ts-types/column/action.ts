...
export type GetRadioEditorGroup<T> = (target: { grid: ListGridAPI<T>; col: number; row: number }) => CellAddress[];
...
export type SortOption<T> = boolean | ((arg: { order: 'asc' | 'desc'; col: number; row: number; grid: ListGridAPI<T> }) => void);
