...
import type {
  ...
  ListGridEventHandlersReturnMap, ListGridSpanBodyOptions,
  MaybePromise,
  MaybePromiseOrUndef,
  Message,
  PasteCellEvent,
...
function _getCellIcon1<T>(grid: ListGrid<T>, icon: ColumnIconOption<T>): ColumnIconOption<T>
function _getCellIcon1<T>(grid: ListGrid<T>, icon: ColumnIconOption<T>[]): ColumnIconOption<T>[]
function _getCellIcon1<T>(grid: ListGrid<T>, icon: ColumnIconOption<T> | ColumnIconOption<T>[]): ColumnIconOption<T> | ColumnIconOption<T>[]
...
function _getHeaderIconTooltip<T>(
  grid: ListGrid<T>,
  col: number,
  row: number
): string {
...
function _onDrawValue<T>(
  ...
  const drawCellBase = (
    { bgColor }: { bgColor?: ColorPropertyDefine } = {}
  ): void => {
    drawCellBg({ bgColor })
    drawCellBorder()
  }
