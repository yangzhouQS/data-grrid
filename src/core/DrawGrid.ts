  ...
  if (keyCode === KEY_LEFT) {
    ...
    if (shiftKey) {
      ...
      if (!hmove.call(this, 'W', false)) {
        // 阻止上移一行
        const row = focusCell.row // this.getMoveUpRowByKeyDownInternal(focusCell)
        if (0 > row) {
          _moveFocusCell.call(this, this.colCount, row, false)
          // return
        } else {
      ...
      if (!hmove.call(this, 'E', false)) {
        // 阻止tab切换下移一行
        const row = focusCell.row // this.getMoveDownRowByKeyDownInternal(focusCell)
        if (this.rowCount <= row) {
          _moveFocusCell.call(this, 0, focusCell.row, false)
        } else {
    ...
    const row = shiftKey ? this.getMoveUpRowByKeyDownInternal(focusCell) : this.getMoveDownRowByKeyDownInternal(focusCell)
    if (row > 0 && row < this.rowCount) {
      _moveFocusCell.call(this, focusCell.col, row, false)
    }
    /*if (shiftKey) {
...
class DrawCellContext implements CellContext {
  ...
  constructor(col: number, row: number, ctx: CanvasRenderingContext2D,
              rect: Rect | null, drawRect: Rect | null, drawing: boolean,
              selection: Selection, drawLayers: DrawLayers
  ) {
    this._col = col
    this._row = row
    this._mode = 0
